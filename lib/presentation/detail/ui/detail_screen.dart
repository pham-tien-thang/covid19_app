...
class _DetailScreen extends State<DetailScreen> with TickerProviderStateMixin {
  ...
  Widget _detailWidget() {
    return BlocConsumer<DetailBloc, DetailState>(listener: (pre, state) async {
      ...
      if (state is AddFavoriteFailStateFromDetail) {
        switch (state.error) {
          case Error.isNotLogin:
            ...
            break;
          case Error.exist:
            showToast("Đã theo dõi quốc gia này", _scaffoldKey.currentContext!,
                Colors.orange, Icons.warning, fToast);
